...
from .main_trainer import BaseTrainer
from preprocess import MixedDataset, BaseDataset
from models import hmr, emo_body_lang, SMPL
from utils.pose_utils import compute_similarity_transform_batch
from utils.geometry_utils import batch_rodrigues, perspective_projection, estimate_translation
from core import path_config, constants
from .fits_dict import FitsDict
from .configs import cfg
from utils.train_utils import print_args
from utils.iuvmap_utils import iuv_img2map, iuv_map2img
...
class Trainer(BaseTrainer):
    ...
    def init_fn(self):
        ...
        if self.options.regressor == 'hmr':
        ...
        elif self.options.regressor == 'emo_body_lang':
            # EBL model
            ...
            self.smpl_female = SMPL(model_path=path_config.SMPL_MODEL_DIR,
                                    gender='female',
                                    create_transl=False).to(self.device)
            self.model = emo_body_lang(path_config.SMPL_MEAN_PARAMS, pretrained=True)
        ...
        if cfg.MODEL.EBL.AUX_SUPV_ON:
            self.iuv_maker = IUV_Renderer(output_size=cfg.MODEL.EBL.DP_HEATMAP_SIZE)
    ...
    def train_step(self, input_batch):
        ...
        if self.options.regressor == 'emo_body_lang' and cfg.MODEL.EBL.AUX_SUPV_ON:
            ...
            iuv_image_gt = torch.zeros((batch_size, 3, cfg.MODEL.EBL.DP_HEATMAP_SIZE, cfg.MODEL.EBL.DP_HEATMAP_SIZE)).to(self.device)
        ...
        len_loop = len(preds_dict['smpl_out']) if self.options.regressor == 'emo_body_lang' else 1

        for l_i in range(len_loop):

            if self.options.regressor == 'emo_body_lang':
                if l_i == 0:
    ...
    def visualize(self, it, target, stage, preds, losses=None):
        ...
        dp_out = preds['dp_out'][-1] if cfg.MODEL.EBL.AUX_SUPV_ON else None
