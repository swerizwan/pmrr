...
def prepare_env(args):
    ...
    sub_dir = 'emo_body_lang_' + cfg.MODEL.EBL.BACKBONE
    if not args.single_dataset:
        sub_dir += '_mix'

    # [backbone]_[pretrained datasets]_[aux.supv.]_[loop iteration]_[time]_[random number]
    log_name = sub_dir
    log_name += '_as_' if cfg.MODEL.EBL.AUX_SUPV_ON else '_'
    log_name += 'lp' + str(cfg.MODEL.EBL.N_ITER)

    if cfg.MODEL.EBL.N_ITER > 0:
        log_name += '_mlp'
        log_name += '-'.join(str(i) for i in cfg.MODEL.EBL.MLP_DIM)
